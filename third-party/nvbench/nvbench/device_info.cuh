...
struct device_info
{
  ...
  { // kHz -> Hz
    int clock_khz = 0;
    cudaDeviceGetAttribute(&clock_khz, cudaDevAttrClockRate, m_id);
    return static_cast<std::size_t>(clock_khz) * 1000;
  }
  ...
  { // kHz -> Hz
    int mem_clock_khz = 0;
    cudaDeviceGetAttribute(&mem_clock_khz, cudaDevAttrMemoryClockRate, m_id);
    return static_cast<std::size_t>(mem_clock_khz) * 1000;
  }

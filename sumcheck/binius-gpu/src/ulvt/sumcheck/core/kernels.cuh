...
__global__ void compute_compositions( // evaluates Si(Xi) at multiple points and gets the claimed sum
	...
	__shared__ uint32_t thread_batches[THREADS_PER_BLOCK][BITS_WIDTH];
	...
	for(int interpolation_point = 0; interpolation_point < INTERPOLATION_POINTS; interpolation_point++) {
		...
		for (int row_idx = tid; row_idx < num_batch_rows; row_idx += gridDim.x * blockDim.x){
			...
			memcpy(thread_batches[threadIdx.x], product, BITS_WIDTH * sizeof(uint32_t));
		}

		__syncthreads();
		for(int stride = THREADS_PER_BLOCK / 2; stride > 0; stride >>= 1) {
			if(threadIdx.x + stride < THREADS_PER_BLOCK) {

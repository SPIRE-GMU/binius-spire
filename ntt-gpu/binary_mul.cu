...
public:
    ...
    static inline __host__ __device__ uint32_t generic_multiply(uint32_t a, uint32_t b) {
		...
		uint32_t z0 = BinaryTower<HEIGHT - 1>::multiply(a0, b0);
		uint32_t z2 = BinaryTower<HEIGHT - 1>::multiply(a1, b1);
		uint32_t z1 = BinaryTower<HEIGHT - 1>::multiply(a0 ^ a1, b0 ^ b1) ^ z0 ^ z2;
		uint32_t z2a = BinaryTower<HEIGHT - 1>::multiply_alpha(z2);
	...
	static inline __host__ __device__ uint32_t generic_square(uint32_t a) {
		...
		auto a0 = aa.first;
		auto a1 = aa.second;
		uint32_t z0 = BinaryTower<HEIGHT - 1>::square(a0);
		uint32_t z2 = BinaryTower<HEIGHT - 1>::square(a1);
		uint32_t z2a = BinaryTower<HEIGHT - 1>::multiply_alpha(z2);
	...
	static inline __host__ __device__ uint32_t generic_inverse(uint32_t a) {
		...
		if (BinaryTower<HEIGHT - 1>::is_valid(a)) return BinaryTower<HEIGHT - 1>::inverse(a);
		auto aa = to_subfield(a);
		auto a0 = aa.first;
		auto a1 = aa.second;
		auto inter = a0 ^ BinaryTower<HEIGHT - 1>::multiply_alpha(a1);
		auto delta = BinaryTower<HEIGHT - 1>::multiply(a0, inter) ^ BinaryTower<HEIGHT - 1>::square(a1);
		auto delta_inv = BinaryTower<HEIGHT - 1>::inverse(delta);
		// Check here this is not zero
		auto inv0 = BinaryTower<HEIGHT - 1>::multiply(delta_inv, inter);
		auto inv1 = BinaryTower<HEIGHT - 1>::multiply(delta_inv, a1);
	...
	static inline __host__ __device__ uint32_t generic_multiply_alpha(uint32_t a) {
		...
		auto a0 = aa.first;
		auto a1 = aa.second;
		uint32_t z1 = BinaryTower<HEIGHT - 1>::multiply_alpha(a1);
...
template <size_t HEIGHT>
class BinaryTower : public BinaryTowerBase<HEIGHT> {
public:

	static inline __host__ __device__ uint32_t multiply(uint32_t a, uint32_t b) {
		return BinaryTowerBase<HEIGHT>::generic_multiply(a, b);
	}

	static inline __host__ __device__ uint32_t square(uint32_t a) {
		return BinaryTowerBase<HEIGHT>::generic_square(a);
	}

	static inline __host__ __device__ uint32_t inverse(uint32_t a) {
		return BinaryTowerBase<HEIGHT>::generic_inverse(a);
	}

	static inline __host__ __device__ uint32_t multiply_alpha(uint32_t a) {
		return BinaryTowerBase<HEIGHT>::generic_multiply_alpha(a);
	}
};

template <>
class BinaryTower<2> : public BinaryTowerBase<2> {
public:
	static inline __host__ __device__ uint32_t multiply(uint32_t a, uint32_t b) {
...
template <>
class BinaryTower<3> : public BinaryTowerBase<3> {
public:
	static inline __host__ __device__ uint32_t multiply(uint32_t a, uint32_t b) {
		return BinaryTowerBase<3>::generic_multiply(a, b);
	}
...
template <>
class BinaryTower<0> : BinaryTowerBase<0> {
public:

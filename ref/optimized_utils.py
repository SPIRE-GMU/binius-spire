...
print("multiplication table works correctly",mul(127, 128))
print("multiplication table works correctly",mul(100, 110))
...
print("value of Wi_eval_cache:", Wi_eval_cache)
...
def additive_ntt(vals):
    ...
    for i in range(log2(size)):
        ...
        print(f"[NTT] After level {i}: {vals[0].reshape(-1)}")  # 打印第一行
        # print(f"[NTT] After level {i}: {vals[1].reshape(-1)}")  # 打印第二行
...
def inv_additive_ntt(vals):
    ...
    for i in range(log2(size)-1, -1, -1):
        ...
        L = vals[..., :, :halflen]
        R = vals[..., :, halflen:]
        print(f"coeff1: {coeff1.flatten()}")
        # print(f"coeff2: {coeff2.flatten()}")
        # print(f"L before: {L[0].reshape(-1)}")
        # print(f"R before: {R[0].reshape(-1)}")
        sub_input1 = mul(L, coeff2) ^ mul(R, coeff1)
        # print(f"sub_input1: {sub_input1[0].reshape(-1)}")
        sub_input2 = L ^ R
        # print(f"sub_input2: {sub_input2[0].reshape(-1)}")
        vals[..., :, :halflen] = sub_input1
        vals[..., :, halflen:] = sub_input2
        print(f"[INTT] After level {i}: {vals[0].reshape(-1)}")  # 打印第一行
        # print(f"[INTT] After level {i}: {vals[1].reshape(-1)}")  # 打印第二行
    return np.reshape(vals, shape_prefix + (size,))
...
def extend(data, expansion_factor=2):
    ...
    # Step 1: Apply inverse additive NTT
    inv_data = inv_additive_ntt(data)
    endtime = time.time() - start_time

    print("endtime for inv_additive_ntt:", endtime)

    print("After inv_additive_ntt:")
    print(inv_data)


    # Step 2: Padding
    pad_width = ([0] * len(data.shape), [0] * (len(data.shape) - 1) + [data.shape[-1] * (expansion_factor - 1)])
    padded = np.pad(inv_data, pad_width)
    print("After padding:")
    print(padded)

    # Step 3: Apply additive NTT
    result = additive_ntt(padded)
    print("After additive_ntt:")
    print(result)

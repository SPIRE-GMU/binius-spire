...
def optimized_binius_proof(evaluations, evaluation_point):
    ...
    # print("evaluations value:", evaluations)
    ...
    # print(f"extended_row_value: {extended_rows}")
...
def optimized_binius_proof_with_profile(evaluations, evaluation_point):
    ...
    log_row_length, log_row_count, row_length, row_count = \
        choose_row_length_and_count(log2(len(evaluations) * 8))
    timings['grid_rearrange']= time.time() - start_time
    start_time = time.time()
    ...
    extended_rows = extend(rows, EXPANSION_FACTOR)
    extended_row_length = row_length * EXPANSION_FACTOR // PACKING_FACTOR
    timings['fft_extend'] = time.time() - start_time
    start_time = time.time()
    ...
    #t_prime = multisubset(row_combination, rows_as_bits_transpose)
    timings['linear_row_combination_t_prime_computation'] = time.time() - start_time
    start_time = time.time()
    ...
    merkle_tree = merkelize(packed_columns)
    root = get_root(merkle_tree)
    timings['merkle_tree'] = time.time() - start_time
    start_time = time.time()

    # Challenge in a few positions, to get branches
    challenges = get_challenges(root, extended_row_length)
    brachnches = [get_branch(merkle_tree, c) for c in challenges]
    timings['challenges and branches'] = time.time() - start_time
    start_time = time.time()
    ...
    # Convert timings to a human-readable format
    timings = {k: f"{v:.6f} seconds" for k, v in timings.items()}
    print("Timings for optimized_binius_proof_with_profile:")

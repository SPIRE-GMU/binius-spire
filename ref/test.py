...
from optimized_binius import (
    optimized_binius_proof, verify_optimized_binius_proof,optimized_binius_proof_with_profile
)
...
from optimized_utils import (
    ...
    np,additive_ntt as op_additive_ntt, inv_additive_ntt as op_inv_additive_ntt
)
...
def test_binary_operations_with_num():
    # Basic arithmetic
    print("Testing basic arithmetic...")
    a = (B(3) + B(14)) * B(15)
    b = B(3) * B(15) + B(14) * B(15)
    print(f"B(3) = {B(3)}, B(14) = {B(14)}, B(15) = {B(15)}")
    print(f"B(3) + B(14) = {B(3) + B(14)}, B(15) = {B(15)}")
    print(f"(B(3) + B(14)) * B(15) = {a}, B(3)*B(15) + B(14)*B(15) = {b}")
    assert a == b

    values = [(B(x) * 3).value for x in range(256)]
    print(f"Unique values when multiplying by 3: {len(set(values))}")
    assert len(set(values)) == 256

    print(f"B(13) ** 2 = {B(13) ** 2}")
    print(f"B(13)**255 = {B(13)**255}")
    assert B(13) ** 255 == 1

    print(f"B(420)**255 = {B(420)**255}")
    assert B(420) ** 255 != 1

    print(f"B(420)**65535 = {B(420)**65535}")
    assert B(420) ** 65535 == 1

    # Polynomial evaluations
    print("Testing polynomial evaluations...")
    evals = [3, 14, 15, B(92)]
    print(f"evals = {evals}")
    print("poly_eval([0,0]) =", multilinear_poly_eval(evals, [0, 0]))
    assert multilinear_poly_eval(evals, [0, 0]) == 3

    print("poly_eval([1,0]) =", multilinear_poly_eval(evals, [1, 0]))
    assert multilinear_poly_eval(evals, [1, 0]) == 14

    print("poly_eval([2,5]) =", multilinear_poly_eval(evals, [2, 5]))
    assert multilinear_poly_eval(evals, [2, 5]) == 204

    # Reed-Solomon extension
    print("Testing Reed-Solomon extension...")
    poly = [B((3**i) % 256) for i in range(8)]
    print("Original polynomial:", poly)

    ntt = op_additive_ntt(poly)
    print("NTT:", ntt)

    evals_ntt = [eval_poly_in_basis(poly, i) for i in range(8)]
    print("Direct evaluation in basis:", evals_ntt)
    assert ntt == evals_ntt

    poly_restored = op_inv_additive_ntt(ntt)
    print("Inverse NTT result:", poly_restored)
    assert poly_restored == poly
...
def test_optimized_binius_with_profile(size):
    z = TEST_DATA_STREAM[:size//8]
    eval_point = [(999**i)%2**128 for i in range(log2(size))]
    proof = optimized_binius_proof_with_profile(z, eval_point)
    print("Generated packed-binius proof")
    print("Proof size: {} bytes".format(compute_size(proof)))
    # print("t_prime:", proof["t_prime"])
    verify_optimized_binius_proof(proof)
...
if __name__ == '__main__':
    ...
    # test_binary_operations_with_num()
    ...
    test_optimized_binius_with_profile(2**31) #use this to test the optimized binius with profile
    # test_optimized_binius(2**12) #use this to build a quick benchmark & testbench for debugging

...
// Multiply by alpha for 4-bit input
...
extern "C" {
void intt_stage0(hls::stream<ap_axis<32, 0, 0, 0> > &input, hls::stream<ap_axis<32, 0, 0, 0> > &output, int size) {
...
for (int i=0; i<size; i++)
    {
        ...
        //   printf("PL kernel read index%d is %d\n",i,in_x.data);
          ...
          value_right = in_x.data & 0xFFFF; //sample.right;
            // printf("value_right=%d|",value_right);
          value_left = (in_x.data >> 16) & 0xFFFF; // sample.left;
            // printf("value_left=%d|",value_left);

          tempR= b_mul_u16(value_right,W[i]);
        //   printf("tempR=%d|",tempR);
          tempL =   b_mul_u16(value_left,W[i]^1);
        //   printf("tempL=%d|",tempL);

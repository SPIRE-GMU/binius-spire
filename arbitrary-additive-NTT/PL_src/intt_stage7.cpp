...
extern "C" {
void intt_stage7(hls::stream<ap_axis<32, 0, 0, 0> > &input, hls::stream<ap_axis<32, 0, 0, 0> > &output, int size) {
...
for (int i=0; i<size; i++)
    {
        ...
        //   printf("PL kernel read index%d is %d\n",i,in_x.data);
          ...
          tempR= b_mul_u16(value_right,W[i/128]);
          tempL =   b_mul_u16(value_left,W[i/128]^1);  //only 1024 coeff

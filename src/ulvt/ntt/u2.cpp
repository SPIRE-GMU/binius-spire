...
static __global__ void coop_antt(AdditiveNTTKernelParams<T> kernel_params, __const__ T *pre_computed)
{
	...
	// Static shared memory for double buffering (16-byte aligned for TMA)
	__shared__ alignas(16) T smem_u[2][BLOCK_SIZE_NEW];
	__shared__ alignas(16) T smem_v[2][BLOCK_SIZE_NEW];
	
	// Two barriers to track the double-buffered pipeline phases
	#pragma nv_diag_suppress static_var_with_dynamic_init
	__shared__ barrier bar[2], out_bar;

	if (is_elected()) {
		init(&bar[0], blockDim.x);
		init(&bar[1], blockDim.x);
		init(&out_bar, blockDim.x);
	}
	__syncthreads();

	int global_warp_id = (blockIdx.x * blockDim.x + threadIdx.x) / 32;
	int partner_warp_id = global_warp_id ^ ((1 << start_stage) / 32);

	int pipe_idx = 0; 
	int next_pipe = 1;

	// Cooperative parallel load of initial data
	int u_row = (global_warp_id * 32) / input_size;
	int u_col_base = (global_warp_id * 32) % input_size;
	int v_row = (partner_warp_id * 32) / input_size;
	int v_col_base = (partner_warp_id * 32) % input_size;
	
	for (int i = threadIdx.x; i < 32; i += blockDim.x) {
		smem_u[pipe_idx][i] = flat_array_2d<T>(data_io, data_pitch, u_row, u_col_base + i);
		smem_v[pipe_idx][i] = flat_array_2d<T>(data_io, data_pitch, v_row, v_col_base + i);
	}
	__syncthreads();

	for (int stage = start_stage; stage >= end_stage; stage--)
	{
		size_t curr_pos = blockIdx.x * blockDim.x + threadIdx.x;
		for(int idx = curr_pos; idx < total_elements; idx += stride) {
			// Cooperative parallel prefetch of next iteration data
			if(idx + stride < total_elements) {
				int next_u_warp_id = ((idx + stride) / 32);
				int next_partner_warp_id = next_u_warp_id ^ ((1 << stage) / 32);
				int next_u_row = (next_u_warp_id * 32) / input_size;
				int next_u_col_base = (next_u_warp_id * 32) % input_size;
				int next_v_row = (next_partner_warp_id * 32) / input_size;
				int next_v_col_base = (next_partner_warp_id * 32) % input_size;
				
				for (int i = threadIdx.x; i < 32; i += blockDim.x) {
					smem_u[next_pipe][i] = flat_array_2d<T>(data_io, data_pitch, next_u_row, next_u_col_base + i);
					smem_v[next_pipe][i] = flat_array_2d<T>(data_io, data_pitch, next_v_row, next_v_col_base + i);
				}
			}
			__syncthreads();

			int butterfly_block_global = (idx % input_size) / (1 << (stage + 1));
			T twiddle = calculate_twiddle<T, P>(pre_computed, kernel_params.constants_pitch, log_h, log_rate, 0, stage, butterfly_block_global);

			T u = smem_u[pipe_idx][threadIdx.x % 32];
			T v = smem_v[pipe_idx][threadIdx.x % 32];
			antt_butterfly<T, P>(u, v, twiddle);
			smem_u[pipe_idx][threadIdx.x % 32] = u;
			smem_v[pipe_idx][threadIdx.x % 32] = v;
			
			__syncthreads();
			
			// Cooperative parallel writeback of v to global memory
			for (int i = threadIdx.x; i < 32; i += blockDim.x) {
				*(_flat_array_2d<T>(data_io, data_pitch, (partner_warp_id * 32 + i) / input_size, (partner_warp_id * 32 + i) % input_size)) = smem_v[pipe_idx][i];
			}
			__syncthreads();
			pipe_idx = 1 - pipe_idx; 
		...
		if(is_elected()) {
			cuda::memcpy_async( _flat_array_2d<T>(data_io, data_pitch, (global_warp_id * 32) / input_size, (global_warp_id * 32) % input_size), smem_u[0], cuda::aligned_size_t<16>(sizeof(T) * 32), out_bar);
		}
		barrier::arrival_token final_token = out_bar.arrive();
		out_bar.wait(std::move(final_token));
		grid.sync();
		pipe_idx = 0;
		next_pipe = 1;
		if(is_elected()) {
			int partner_warp_id = global_warp_id ^ ((1 << (stage-1)) / 32);
			cuda::memcpy_async(smem_u[pipe_idx], _flat_array_2d<T>(data_io, data_pitch, (global_warp_id * 32) / input_size, (global_warp_id * 32) % input_size), cuda::aligned_size_t<16>(sizeof(T) * 32), bar[pipe_idx]);
			cuda::memcpy_async(smem_v[pipe_idx], _flat_array_2d<T>(data_io, data_pitch, (partner_warp_id * 32) / input_size, (partner_warp_id * 32) % input_size), cuda::aligned_size_t<16>(sizeof(T) * 32), bar[pipe_idx]);
		}
	}
	grid.sync();
}
	...
	for (int stage = start_stage; stage >= end_stage; stage--)
	{
		for (size_t pos = global_tid; pos < total_elements; pos += stride)
		{
			...
			if (partner_pos > pos)
			{
				...
				// Direct pointer arithmetic to avoid repeated flat_array_2d calls
...
static __global__ void antt_inwarp(AdditiveNTTKernelParams<T> kernel_params, __const__ T *pre_computed)
{
	...
	// Compute stages in shared memory
	for (int stage = start_stage; stage >= end_stage; stage--)
	{
		// Skip stages that are >= log_h (not valid for this problem size)
		if (stage >= log_h)
			continue;
...
public:
	...
	bool apply(const NTTData<T> &input, NTTData<T> &output)
	{
		...
		// const int top_stage = log_h + log_rate - 1;
		// const int coop_start_stage = 11;
		// const int dsmem_start_stage = 10;
		// const int dsmem_end_stage = 9;
		// const int inwarp_start_stage = 8;

		// // Launch coop_antt first for the highest stages.
		// if (top_stage >= coop_start_stage) {
		...
		// 	kernel_params.end_stage = dsmem_end_stage;//coop_start_stage;
		...
		// // Then run antt_dsmem for the middle stages.
		// if (top_stage >= dsmem_end_stage) {
		// 	int blocks = ((sm_core_count * 3) / CLUSTER_SIZE_CONST) * CLUSTER_SIZE_CONST;
		// 	if (blocks == 0) blocks = CLUSTER_SIZE_CONST;

		// 	kernel_params.start_stage = min(dsmem_start_stage, top_stage);
		// 	kernel_params.end_stage = dsmem_end_stage;

		// 	cudaLaunchConfig_t config = {};
		// 	config.gridDim = dim3(blocks, 1, 1);
		// 	config.blockDim = dim3(BLOCK_SIZE_NEW, 1, 1);
		// 	config.dynamicSmemBytes = 2 * BLOCK_SIZE_NEW * sizeof(T);
		...
		// 	int max_clusters = 0;
		// 	CUDA_CHECK(cudaOccupancyMaxActiveClusters(&max_clusters, antt_dsmem<T, P>, &config));
		// 	if (max_clusters > 0) {
		// 		const int max_blocks = max_clusters * CLUSTER_SIZE_CONST;
		// 		if (blocks > max_blocks) blocks = max_blocks;
		// 		blocks = (blocks / CLUSTER_SIZE_CONST) * CLUSTER_SIZE_CONST;
		// 		if (blocks == 0) blocks = CLUSTER_SIZE_CONST;
		// 	}
		// 	config.gridDim = dim3(blocks, 1, 1);

		// 	CUDA_CHECK(cudaLaunchKernelEx(&config, antt_dsmem<T, P>, kernel_params, pre_computed));

		// }

		// Finally, run antt_inwarp for the lowest stages.
		// if (top_stage >= 0) {
		// 	kernel_params.start_stage = std::min(inwarp_start_stage, top_stage);
		// 	kernel_params.end_stage = 0;
		...
		// FIX: The number of butterfly stages is strictly defined by the input size (log_h).
		// The coset extension (log_rate) alters twiddle calculations, NOT the number of stages.
		const int top_stage = log_h - 1; 
		
		// Calculate the maximum stage antt_inwarp can safely handle.
		// It uses 512 shared memory elements, so max stage is 8 (2^8 = 256 shift).
		int inwarp_start_stage = std::min(top_stage, 4);

		// Run coop_antt for ALL higher stages down to the exact hand-off point
		...
		// Run antt_inwarp for the remaining lower stages entirely inside shared memory

...
protected:
	...
	static inline __host__ __device__ uint32_t generic_inverse(uint32_t a)
	{
		...
		if (FanPaarTowerField<HEIGHT - 1>::is_valid(a))
			return FanPaarTowerField<HEIGHT - 1>::inverse(a);
		auto aa = to_subfield(a);
		auto a0 = aa.first;
		auto a1 = aa.second;
...
{
public:
	static inline __host__ __device__ uint8_t gf4_mul(uint8_t a, uint8_t b)
	{
	...
	static inline __host__ __device__ uint8_t gf16_mul(uint8_t A, uint8_t B)
	{
		uint8_t a0 = A & 3;
		uint8_t a1 = (A >> 2) & 3;
		uint8_t b0 = B & 3;
		...
		uint8_t a0b1 = gf4_mul(a0, b1);
		uint8_t a1b0 = gf4_mul(a1, b0);
		uint8_t a1b1 = gf4_mul(a1, b1);

		uint8_t u = 2;
		uint8_t v = 1;

		uint8_t term0 = a0b0 ^ gf4_mul(a1b1, v);
		uint8_t term1 = a0b1 ^ a1b0 ^ gf4_mul(a1b1, u);

		return (term0 & 3) | ((term1 & 3) << 2);
	}
